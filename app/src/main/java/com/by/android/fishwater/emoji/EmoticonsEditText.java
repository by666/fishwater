...
public class EmoticonsEditText extends EditText {
    ...
    public EmoticonsEditText(Context context, AttributeSet attrs) {
        ...
        if (mEmoticonBeanList == null) {
            DBHelper dbHelper = new DBHelper(mContext);
            mEmoticonBeanList = dbHelper.queryAllEmoticonBeans();
            dbHelper.cleanup();
        }
        if (mEmoticonBeanList == null) {
            mEmoticonBeanList = new ArrayList<>();
        }
    }
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int after) {
        ...
        if (after > 0) {
            ...
            for (EmoticonBean bean : mEmoticonBeanList) {
        ...
        if (onTextChangedInterface != null) {
            onTextChangedInterface.onTextChanged(text, start, lengthBefore, after);
        }
    }
