...
public class Utils {
     ...
     * 键盘默认高度 (dp)
     */
    private static int sDefKeyboardHeight = 276;
    public static int sDefRow = 7;
    public static int sDefLine = 3;

    public static boolean isInitDb() {
        return SettingFlags.getBooleanFlag(EXTRA_ISINITDB);
    }

    public static void setIsInitDb(boolean b) {
        SettingFlags.setFlag(EXTRA_ISINITDB, b);
    }

    public static int getDefKeyboardHeight() {
        int height = SettingFlags.getIntFlag(EXTRA_DEF_KEYBOARDHEIGHT, 0);
        if (height > 0 && sDefKeyboardHeight != height) {
            Utils.setDefKeyboardHeight(height);
        }
    ...
    public static void setDefKeyboardHeight(int height) {
        if (sDefKeyboardHeight != height) {
            SettingFlags.setFlag(EXTRA_DEF_KEYBOARDHEIGHT, height);
        }
